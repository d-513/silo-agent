...
The execution engine is shared by chats, channels, and future automations (`startRun` → `runLoop`). The model ends a user-visible block with `<section_send />`; each section becomes its own chat bubble and, on a channel, is delivered immediately. Thinking and tool output are never delivered to a channel. The `channel` chat tool sends to a channel (default the origin one) gated per channel (`channels.<channelID>`); `chats` reads this Bot's conversations (`chats.read`). `present` and `artifact` on a channel attach the real file (a skill directory is zipped), via the adapter's outbound `Files`. Python reaches the same through `silo_runtime.send_channel` / `read_chats`. Telegram credits live per channel (`api_id`/`api_hash`/`bot_token`); the MTProto session is stored under `data/channels/<id>/` on the Control Plane.
