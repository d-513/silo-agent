...
| Operator config | Koanf: defaults → YAML → env (`SILO_FOO__BAR` → `foo.bar`) | listen addr, sqlite path, docker host, per-provider keys, model allowlist + default, search, STDIO MCP image. Admin Settings writes `silo.yaml`. |
