...
The system prompt is ordered stable→volatile in cache tiers: base + identity, session config (connectors, skills, channels), SOUL, MEMORY, then the per-run channel note and "Recalled memories". Memory has two tiers: MEMORY (`bots.memory`, 8k, always in the prompt) and long-term `memories` rows (pgvector `vector(1536)`, HNSW cosine, per bot). `remember` embeds with `embedding_model` and updates its nearest neighbour instead of adding a near-copy (distance < 0.05); `recall` is top-k by `<=>`; `forget` deletes by id. When `memory.auto_recall` is on, the opening message is embedded once per run and up to 3 memories within distance 0.65 go into the trailing block, after every cache breakpoint. Prompt caching is opt-in per provider (`providers.<id>.cache`); Anthropic gets `cache_control` breakpoints at those tier boundaries and on the last message, OpenAI/OpenRouter get `prompt_cache_key`. Usage (including cache read/write) is emitted as a `usage` run event and shown as a chip in the composer.
