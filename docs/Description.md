...
The system prompt is ordered stable→volatile in cache tiers: base + identity, session config (connectors, skills, channels), SOUL, MEMORY, then the per-run channel note. Prompt caching is opt-in per provider (`providers.<id>.cache`); Anthropic gets `cache_control` breakpoints at those tier boundaries and on the last message, OpenAI/OpenRouter get `prompt_cache_key`. Usage (including cache read/write) is emitted as a `usage` run event and shown as a chip in the composer.
