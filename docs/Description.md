...
| Piece | Choice | Why |
|---|---|---|
| Control Plane | Go | one binary, Docker API, Postgres, ConnectRPC |
...
| DB | Postgres 17 + pgvector, GORM (pgx) | AutoMigrate, no repository layer; sqlc only if GORM hurts. Every written string is scrubbed (invalid UTF-8, NUL). Run events order by `seq` |
| Auth | session cookies now | OIDC later creates the same `User`/`Session` rows — no plugin framework |
| Operator config | Koanf: defaults → YAML → env (`SILO_FOO__BAR` → `foo.bar`) | listen addr, database URL, docker host, per-provider keys, model allowlist + default, search, STDIO MCP image. Admin Settings writes `silo.yaml`. |
| Product libraries | Postgres + `data/skills/` | Connectors Library, Skills Library. Not YAML. |
...
First-class tools the model sees stay small: `exec_python`, `terminal`, files (`read`/`write`/`patch`/`delete`/`grep`), `present`, `skill` / `artifact`, `web_search`. `read` is numbered and sliced on the Worker (`offset`/`limit`, binary-rejecting, `next_offset` continuation; the displayed line number is the file's absolute line, 1-based). `patch` requires a unique `old_text`. `delete` removes a file or directory recursively. `grep` is ripgrep-backed (`include` glob, `max_hits` cap, deterministic order, no shell). A repeat guard stubs a 3rd identical `read`/`grep` in a run. `present` of a user-facing path is a folio in the thread. `present` of `bot/…` is scratch: collapsed “Looked at …” row, same pixels to the model. CP uses `browse_file`; the tool text is a short ack. Images (`png`/`jpg`/`webp`/`gif`) attach as a multimodal part on the next completion. No base64 in the tool text or the run log. Connectors are discovered on disk as `tools.*`. Skills follow the [Agent Skills](https://agentskills.io/specification) format (`SKILL.md`); bodies live on the CP under `data/skills/` (library + per-user personal). Enablement is the `bot_skills` table. The prompt lists name + description only; `skill` loads the markdown; extras sync to `/opt/silo/skills/<name>/`. Every gated action (`python.run`, `terminal.run`, `files.*`, `desktop.*`, `bot.soul`/`core_memory`/`remember`/`recall`/`forget`, `skills.load`, `artifact.emit`, `web.search`, `model.list`/`switch`, `secrets.<name>`, MCP `slug.action`) goes through one `authorizeAction`: bot rule, else `*` for that connector, else the builtin catalog default, else the MCP connector default, else ask. Python and Terminal default allow. `skills.load`, `artifact.emit`, and `web.search` default allow. `artifact` emits a thread card for a skill directory (with `SKILL.md`) or any file; **Save skill** on a skill card copies the workspace dir to personal — there is no Ask slip that installs. `present` is separate: it shows a file inline and never produces a card. Each secret is its own action — not `secrets.get`. Chat tools and Python `CallTool` share that gate. `desktop.*` from Python is grant-then-local on the Worker so typed secrets never ride to the CP. `web.search` runs on the CP (engine registry); Python reaches it via `silo_runtime.web_search`.
...
STDIO connectors run **outside** the Bot: one isolated sidecar container per attachment (`silo-mcp-<botConnectorID>`), on the same Docker engine as Bots. The sidecar image (`mcp_stdio_image`, default `localhost/silo-mcp-stdio:v1`) includes Node/`npx`, Python, and `silo-mcp-bridge`. A library preset may name a curated image that still ships that bridge as entrypoint. The bridge starts exactly one child from structured `command` + `args` (never a shell) and **dials the CP** over ConnectRPC `MCPHost.Tunnel`, authenticating with a per-attachment bridge token so the CP knows which Bot/connector it is. Raw JSON-RPC lines are tunneled both ways between the child's stdin/stdout and the CP's `mcpx` MCP client, so authorization, audit, masking, and tool stubs are identical to HTTP MCPs. There is no published port and no bind-mounted socket. The bridge token is stripped from the child's env. Env values are CP-side (plaintext or a Bot secret name, resolved only at sidecar create). Docker inspect of the sidecar can still see injected environment — treat that as the v1 ceiling. Sidecars get no Bot volumes and no bot token, but share an npm cache volume so recreating one does not re-download packages. Connecting and tool discovery run in the background: the attachment shows `initializing` (with a status detail) until it is `authorized` or `error`; a CP restart does not recreate sidecars, their bridges simply reconnect. A CP startup sweep (`reconcileStdio`) reclaims any `silo-mcp-*` container whose attachment row is gone. Custom STDIO command/args are allowed for the Bot owner; only an admin can set `stdio_image`.

### Channels

Channels are built-in ways to talk to a Bot: adapters live in `internal/channels` (registry + `Adapter`/`Host` contract) with the first implementation `internal/channels/telegram` (gotd/td MTProto, bot login). A channel row is one attached adapter instance per Bot, unique by name; a Bot may attach several, including two of the same adapter. Adapter config is declarative (`Descriptor.Fields`), but an adapter can also publish dynamic `State` (a QR code, a connection status, a chat picker populated by `Action "list_dialogs"`) — the UI renders it generically, so a future WhatsApp adapter needs no proto or form change.

Channel conversations are ordinary `Chat` rows tagged `channel_id` + `external_id`; Web UI chats keep `channel_id=''` and stay web-only. A channel with `inbound=false` is send-only. Per-channel secrets are `Secret` rows named `channel.<id>.<key>`, hidden from the Secrets tab and the model's `secrets.*` action but included in the masker. Channel workers start from `reconcileChannels`; `Adapter.Start` is live-only — it never backfills messages missed while offline, so a Bot that was down does not replay a backlog of runs. Every inbound message goes through `startOrInject`: if a run is already live for that conversation it is injected into that run's inbox, otherwise a run starts and, if the box is stopped, the container is started and worker-backed tools wait for the worker.

The execution engine is shared by chats, channels, and future automations (`startRun` → `runLoop`). The model ends a user-visible block with `<section_send />`; each section becomes its own chat bubble and, on a channel, is delivered immediately. Thinking and tool output are never delivered to a channel. The `channel` chat tool sends to a channel (default the origin one) gated per channel (`channels.<channelID>`); `chats` reads this Bot's conversations (`chats.read`), preferring the adapter's platform history and falling back to the local log when the platform refuses it (Telegram bots: `BOT_METHOD_INVALID`). `present` and `artifact` on a channel attach the real file (a skill directory is zipped), via the adapter's outbound `Files`. Python reaches the same through `silo_runtime.send_channel` / `read_chats`. Telegram credits live per channel (`api_id`/`api_hash`/`bot_token`); the MTProto session is stored under `data/channels/<id>/` on the Control Plane.

### Model engine

`internal/llm` is a provider registry over a neutral request/stream shape; the agent loop never imports a provider SDK. Providers are `openrouter`/`openai` (OpenAI-compatible) and `anthropic` (native Messages API). A model id is `provider/model`; `models` is the operator allowlist, `model` the default, and a `Chat` may override via `chats.model` (the composer selector and the bot's `switch_model` tool, gated `model.switch`). `list_models` (`model.list`) reports the allowlist. Chats share one history across providers.

The system prompt is ordered stable→volatile in cache tiers: base + identity, session config (connectors, skills, channels), SOUL, CORE MEMORY, then the per-run channel note and "Recalled memories". Memory has two tiers: CORE MEMORY (`core_memory` tool, `bots.memory`, 8k, always in the prompt) and long-term `memories` rows (pgvector `vector(1536)`, HNSW cosine, per bot). `remember` embeds with `embedding_model` and updates its nearest neighbour instead of adding a near-copy (distance < 0.05); `recall` is top-k by `<=>`; `forget` deletes by id. The Bot's **Memories** tab edits CORE MEMORY and lists long-term memories; `SearchMemories` ranks them by meaning for the human without bumping `last_used_at`. When `memory.auto_recall` is on, the opening message is embedded once per run and up to 3 memories within distance 0.65 go into the trailing block, after every cache breakpoint. Prompt caching is opt-in per provider (`providers.<id>.cache`); Anthropic gets `cache_control` breakpoints at those tier boundaries and on the last message, OpenAI/OpenRouter get `prompt_cache_key`. Usage (including cache read/write) is emitted as a `usage` run event and shown as a chip in the composer.


### Chromium

Not autostarted with the desktop. The dock **Chromium** item and `silo-chromium` launch a headed browser on `DISPLAY=:1`, persistent `--user-data-dir=/home/silo/chrome-profile`, CDP on `127.0.0.1:9222`, a native 1600×900 Xvfb, and uBlock Origin Lite (managed policy: ads, cookie banners, overlays). Openbox (`botimage/openbox/rc.xml`) maximizes every Chromium window on map, so it fills the WM work area above the tint2 dock while keeping its decorations — it is maximized, not fullscreen. Closing Chromium must not take down the container. GUI work uses first-class `look` / `click` / `type` / `key` / `scroll` on the 1600×900 X11 desktop (screenshot pixels = display pixels; no scale) — that is the live click path. Playwright `silo_runtime.chrome_page()` is for page screenshots, mutating displayed HTML, and automated scripts. The worker opens `silo-chromium` when `exec_python` mentions Playwright / `chrome_page`; `chrome_page` also hits `POST /v1/chrome/ensure`. Do not `playwright install` a second browser. The model must not call `pyautogui` or `xdotool`; the worker may. The desktop, console, and worker run as user `silo` (uid 1000), not root; `sudo` is passwordless.
